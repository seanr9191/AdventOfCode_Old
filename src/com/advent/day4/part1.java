...
public class part1 {
    ...
    public void solve() throws FileNotFoundException {
        File input = new File("src/com/advent/day4/part1.txt");
                ...
                for (int i = 0; i < 5; i++) {
                    for (int p = 0; p < currBoard[i].length; p++) {
                        if (currBoard[i][p] == numbers.get(numIndex)) {
                            currBoard[i][p] *= -1;
                            if (isBingo(currBoard, i, p) && (numIndex+1 < fastestBingo || fastestBingo == -1)) {
                                fastestBingo = numIndex+1;
        ...
        System.out.println(String.format("Score: %d, Number of Turns: %d", bestScore, fastestBingo));
    }
}
